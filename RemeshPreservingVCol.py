bl_info = {
    "name": "Remesh Preserving VCol",
    "author": "terracottahaniwa",
    "version": (1, 0),
    "blender": (2, 80, 0),
    "location": "Call from F3 Search",
    "description": "Remesh Preserving VCol",
    "category": "Remesh",
}
...
def main(self, context):
    if context.active_object.type == 'MESH':
        ...
        temporal_modifire.data_types_loops = {'VCOL'}
...
class REMESH_OT_PreservingVCol(bpy.types.Operator):
    """Remesh Preserving VCol"""
    bl_idname = "object.remesh_preserving_vcol"
    bl_label = "Remesh Preserving VCol"
...
def menu_func(self, context):
    self.layout.operator(REMESH_OT_PreservingVCol.bl_idname, icon='MOD_REMESH')


def register():
    bpy.utils.register_class(REMESH_OT_PreservingVCol)
    bpy.types.TOPBAR_MT_app_system.append(menu_func)


def unregister():
    bpy.utils.unregister_class(REMESH_OT_PreservingVCol)
    bpy.types.TOPBAR_MT_app_system.append(menu_func)
